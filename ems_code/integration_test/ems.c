...
void send_setpoints(struct system* sys, struct structSpiQueue* tx_buffer) {
    spiQueuePostInt(tx_buffer, SETPOINT_BATTERY1_ID, sys->goat_preference->battery_power[0]);
    spiQueuePostInt(tx_buffer, SETPOINT_BATTERY2_ID, sys->goat_preference->battery_power[1]);
    spiQueuePostInt(tx_buffer, SETPOINT_DG1_ID, sys->goat_preference->dg_power[0]);
    spiQueuePostInt(tx_buffer, SETPOINT_DG2_ID, sys->goat_preference->dg_power[1]);
}
