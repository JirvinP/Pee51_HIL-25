...
struct ship_state_subroutines subroutines[] = {
    {.mode = SAIL_EMPTY,
     .subroutine = sail_subroutine,
     .reference_power = IDLE_POWERSETPOINT,
     .name = "sailing empty"},
    {.mode = DEPLOY_GEAR,
     .subroutine = deploy_subroutine,
     .reference_power = DEPGEAR_SETPOINT,
     .name = "deploying gear"},
    {.mode = TRAIL,
     .subroutine = trail_subroutine,
     .reference_power = TRAIL_SETPOINT,
     .name = "trailing"},
    {.mode = COLLECT_GEAR,
     .subroutine = collect_subroutine,
     .reference_power = COLLGEAR_SETPOINT,
     .name = "collecting gear"},
    {.mode = SAIL_FULL,
     .subroutine = sail_subroutine,
     .reference_power = IDLE_POWERSETPOINT,
     .name = "sailing full"},
    {.mode = RAINBOW,
     .subroutine = rainbow_subroutine,
     .reference_power = RAIN_POWERSETPOINT,
     .name = "rainbowing"},
    {.mode = SHORE_PUMP,
     .subroutine = shpump_subroutine,
     .reference_power = SHPUMP_POWERSETPOINT,
     .name = "shore pumping"},
    {.mode = DUMPING,
     .subroutine = dump_subroutine,
     .reference_power = DUMP_POWERSETPOINT,
     .name = "dumping"},
    {.mode = BUNKERING,
     .subroutine = bunk_subroutine,
     .reference_power = BUNK_SETPOINT,
     .name = "bunkering"}};
 ...
 *
 *   returns: 0 -> for inefficient calcs, 1 -> for soc calcs, 2 -> for fuel
 * efficiency calcs, 3 -> charge batt1, 4 -> charge batt2, 5 -> charge both
 */
...
int ems_rule_check(struct system *sys) {
  if (sys->charge_on == true) {
    if (sys->user_setting == SOC) {
      if (sys->battery_soc[0] < CHARGE_COMPLETE_SOC_45 &&
          sys->battery_soc[1] < CHARGE_COMPLETE_SOC_45) {
      ...
      if (sys->battery_soc[0] < CHARGE_COMPLETE_SOC_35 &&
          sys->battery_soc[1] < CHARGE_COMPLETE_SOC_35) {
        return CHARGE_BATTERY_BOTH;
      }
  ...
  if (sys->user_setting == SOC) {
    if (sys->battery_soc[0] < CHARGING_SOC &&
        sys->battery_soc[1] < CHARGING_SOC) {
      sys->charge_on = true;
      return CHARGE_BATTERY_BOTH;
    }
...
void ready_setpoint(struct system *sys, int ems_state) {
  if (ems_state == CHARGE_BATTERY_1 || ems_state == CHARGE_BATTERY_BOTH ||
      ems_state == CHARGE_BATTERY_2) {
  ...
  case INEFFICIENT_CALCULATIONS: {
    sys->goat_preference->dg_power[0] =
        (sys->goat_preference->total_power * TWENTYFIVE_PERCENT);
    sys->goat_preference->dg_power[1] =
        (sys->goat_preference->total_power * TWENTYFIVE_PERCENT);
    sys->goat_preference->battery_power[0] =
        (sys->goat_preference->total_power * TWENTYFIVE_PERCENT);
    sys->goat_preference->battery_power[1] =
        (sys->goat_preference->total_power * TWENTYFIVE_PERCENT);
    break;
  }
  case SOC_CALCULATIONS: {
    ...
    if (sys->goat_preference->total_power == BUNK_SETPOINT) {
      sys->goat_preference->dg_power[0] =
          (sys->goat_preference->total_power * FIFTY_PERCENT);
      sys->goat_preference->dg_power[1] =
          (sys->goat_preference->total_power * FIFTY_PERCENT);
    ...
    if (sys->goat_preference->total_power >= IDLE_POWERSETPOINT) {
      sys->goat_preference->dg_power[0] =
          (sys->goat_preference->total_power * FOURTY_PERCENT);
      sys->goat_preference->dg_power[1] =
          (sys->goat_preference->total_power * FOURTY_PERCENT);
      sys->goat_preference->battery_power[0] =
          (sys->goat_preference->total_power * TEN_PERCENT);
      sys->goat_preference->battery_power[1] =
          (sys->goat_preference->total_power * TEN_PERCENT);
    }
  ...
  case FUEL_EFFICIENCY_CALCULATIONS: {
    ...
    if (sys->goat_preference->total_power >= IDLE_POWERSETPOINT &&
      ...
      sys->goat_preference->dg_power[1] = 0;
      sys->goat_preference->battery_power[0] =
          (sys->goat_preference->total_power - 1900) / 2;
      sys->goat_preference->battery_power[1] =
          (sys->goat_preference->total_power - 1900) / 2;
    }
  ...
  case CHARGE_BATTERY_1: {
    ...
    unsigned int remaining_available_power =
        MAX_POWER_DG - sys->goat_preference->total_power;
    if (remaining_available_power > 1000) {
      sys->goat_preference->battery_power[0] = -1000;
      sys->goat_preference->dg_power[0] =
          (sys->goat_preference->total_power * FIFTY_PERCENT) + 500;
      sys->goat_preference->dg_power[1] =
          (sys->goat_preference->total_power * FIFTY_PERCENT) + 500;
    } else {
  ...
  case CHARGE_BATTERY_BOTH: {
    ...
    if (remaining_available_power > 2000) {
      ...
      sys->goat_preference->battery_power[1] = -1000;
      sys->goat_preference->dg_power[0] =
          (sys->goat_preference->total_power * FIFTY_PERCENT) + 1000;
      sys->goat_preference->dg_power[1] =
          (sys->goat_preference->total_power * FIFTY_PERCENT) + 1000;
    } else {
...
void send_setpoints(struct system *sys) {
  // spiQueuePost(tx_buffer, SETPOINT_BATTERY1_ID,
  // sys->goat_preference->battery_power[0]); spiQueuePost(tx_buffer,
  // SETPOINT_BATTERY2_ID, sys->goat_preference->battery_power[1]);
  // spiQueuePost(tx_buffer, SETPOINT_DG1_ID,
  // sys->goat_preference->dg_power[0]); spiQueuePost(tx_buffer,
  // SETPOINT_DG2_ID, sys->goat_preference->dg_power[1]);
}
